...
<fullquery name="select_items_month">
  <querytext>
      select   
      	     to_char(t.start_date, 'YYYY-MM-DD HH24:MI:SS') as ansi_start_date,
             to_char(t.end_date, 'YYYY-MM-DD HH24:MI:SS') as ansi_end_date,
             to_char(pstart_date, 'YYYY-MM-DD HH24:MI:SS') as p_ansi_start_date,
             to_char(pend_date, 'YYYY-MM-DD HH24:MI:SS') as p_ansi_end_date,
             to_number(to_char(t.start_date,'HH24'),'90') as start_hour,
             to_number(to_char(t.start_date,'MI'),'90') as start_minutes,
             to_number(to_char(t.start_date,'SSSSS'),'99990') as start_seconds,
             to_number(to_char(t.end_date,'HH24'),'90') as end_hour,
             to_number(to_char(t.end_date,'MI'),'90') as end_minutes,
             to_number(to_char(t.end_date,'SSSSS'),'99990') as end_seconds,
             coalesce(e.name, a.name) as name,
             coalesce(e.status_summary, a.status_summary) as status_summary,
             coalesce(e.description, a.description) as description,
             e.event_id as item_id,
    ...
    from     
             acs_activities a,
             acs_events e
			 LEFT JOIN (Select start_date as pstart_date, end_date as pend_date, project_name From im_projects) p on p.project_name = e.name,
             timespans s,
             time_intervals t,
             calendars cals,
             cal_items ci left join
